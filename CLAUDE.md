...
- A push to `aigamma/about.aigamma.com/main` does **not** automatically re-ingest. The aigamma.com workflow does not yet listen for events from this repo (cross-repo `repository_dispatch` is not wired; it would require a fine-grained PAT held as a secret in this repo).
- Staleness window: **up to ~24 hours** for an about-page prose edit to land in the chat corpus, via the next aigamma.com cron tick.
- **Faster turnaround**: from the aigamma.com Actions tab, click "Refresh RAG corpus" -> Run workflow. Or `gh workflow run refresh-rag.yml --repo aigamma/aigamma.com`. Either trigger checks out the current `main` of this repo and re-ingests the latest prose immediately.
