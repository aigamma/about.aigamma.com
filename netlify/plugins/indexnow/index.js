...
// Bing's IndexNow API on every successful production deploy. Parses the
// sitemap shipped at the publish root (sitemap.xml), extracts the <loc>
// entries, and POSTs them to https://api.indexnow.org/indexnow in batches
// of up to 10000 URLs per request (the protocol maximum). The IndexNow
// protocol is a unified push notification API that Bing, Yandex, Naver,
// Seznam, and IndexNow.org-aligned engines all consume from a single
// submission, so one POST per deploy reaches every participating index.
//
// Mirrors the equivalent plugin at netlify/plugins/indexnow/ in the
// aigamma.com repo so future maintenance reads identically across both
// surfaces.
//
// Required env vars (set in Netlify UI under Site settings ->
// Environment variables on the aboutaigamma project, never in code or
// netlify.toml):
//   INDEXNOW_API_KEY - the 32-character lowercase-hex key that matches the
//                      verification file at https://about.aigamma.com/<key>.txt
//   SITE_HOST        - the bare host: about.aigamma.com
//
...
export const onSuccess = async ({ utils, constants }) => {
  ...
  if (!host) {
    return utils.build.failPlugin(
      'SITE_HOST env var is not set. Configure it under Netlify Site settings -> Environment variables (value should be the bare host: about.aigamma.com).'
    )
  }
  ...
  try {
    ...
    return utils.build.failPlugin(
      `Could not read sitemap at ${sitemapPath}: ${err.message}. Verify sitemap.xml exists at the repo root.`
    )
  }
    ...
    if (!res.ok) {
      ...
      return utils.build.failPlugin(
        `IndexNow batch ${batchNum}/${batchCount} returned ${res.status} ${res.statusText}. Response body: ${text || '(empty)'}. ` +
          `A 403 typically means the key file at ${keyLocation} is not publicly readable; verify the file exists at the repo root and re-deploy.`
      )
    }
